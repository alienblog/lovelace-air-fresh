...
class AirFilter extends HTMLElement {
  set hass(hass) {
    ...
    if (!this.card) {
      ...
      // div.querySelector('.bg-on').appendChild(container)

      // 定义事件
      ui.querySelector('.pm').onclick = () => {
        this.log('开关')
  ...
  .pm{border-radius:50%;border:4px solid #01be9e;width:150px;height:150px;margin:10px auto;cursor: pointer; position: relative;}
  .pm p:nth-child(1){margin-top:35px;}
  .pm p:nth-child(2){font-size:50px;}
  .pm.active{background:#000;color: white;}
  .pm .bg-on{display: none;}
  .pm.active .bg-on{display: block;}
  .bg-on{position: relative;top:0;width:100%;height:100%;overflow: hidden;border-radius: 50%;}
  .con{position: absolute;top:0;width:100%;height:100%;}
  ...
  .op-row{display:flex;padding:10px;background:white;}
  .op-row .op{width:100%;}
  ...
  .level-1 .op-row{border-top: 3px #01be9e solid !important;}
  ...
  .level-2 .op-row{border-top: 3px #01be9e solid !important;}
  ...
  .level-3 .op-row{border-top: 3px orange solid !important;}
  ...
  .level-4 .op-row{border-top: 3px blue solid !important;}
  ...
  .level-4 .op-row{border-top: 3px red solid !important;}
  ...
  setUI(airboxb, { title, mode, aqi,
    ...
    if (aqi < 50) {
      ...
      quality = '良'
      airboxb.classList.add('level-2')
    }
    else if (aqi < 150) {
      quality = '轻度污染'
      airboxb.classList.add('level-3')
    }
    else if (aqi < 200) {
      quality = '中度污染'
      airboxb.classList.add('level-4')
    }
    else if (aqi < 300) {
      quality = '重度污染'
      airboxb.classList.add('level-5')
    }
    else {
      quality = '严重污染'
    ...
    if (state === 'on') {
      ...
      // airboxb.querySelector('.bg-on').removeChild(airboxb.querySelector('.container'))
    }
    // 自动模式
    activeElement = airboxb.querySelector('.var-auto')
    if (mode == 'auto') {
      if (activeElement.classList.contains('active') === false) {
        activeElement.classList.add('active')
      }
